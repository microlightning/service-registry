...
var requestLifeCycleEvents = {
    ...
    beginRequest: function(req, res, next) {
        ...
        //     res.status(400).send({ error: "'x-jetstream-trace-id' header not provided" })
        // } else if (!req.headers['x-jetstream-source']) {
        //     res.status(400).send({ error: "'x-jetstream-source' header not provided" })
        // } else {
    ...
    // fired at the very end of a request, after anything else, including the result being sent
